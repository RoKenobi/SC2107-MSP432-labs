...
int main(void)
{
    ...
    {                     // Loop forever
        ...
        EUSCIA0_OutChar(CR); EUSCIA0_OutChar(LF);
        EUSCIA0_OutString("[15] Switch Control LED Demo");
        EUSCIA0_OutChar(CR); EUSCIA0_OutChar(LF);
        ...
        EUSCIA0_OutChar(CR); EUSCIA0_OutChar(LF);
        EUSCIA0_OutString("[19] Sequenced Wall Follow");
        EUSCIA0_OutChar(CR); EUSCIA0_OutChar(LF);
        EUSCIA0_OutString("[20] Cup Navigation not working");
        EUSCIA0_OutChar(CR); EUSCIA0_OutChar(LF);
        EUSCIA0_OutString("[21] LF case 1 need to check");
        EUSCIA0_OutChar(CR); EUSCIA0_OutChar(LF);
        EUSCIA0_OutString("[22] LF case 2 need to check ");
        EUSCIA0_OutChar(CR); EUSCIA0_OutChar(LF);
...
void AppTurnRight90(void) {
    ...
    Motor_Right(3000, 3000);
    Clock_Delay1ms(750); // Calibrate this delay for your RSLK
    Motor_Stop();
    ...
    TimedPause(800);
     ...
     while(1){
       ...
       // STARTING , ADJUST DISTANCE AND TURN LEFT
         while ( CenterConvert(nc) > 200 && firstStart)
         {
         ...
         TimedPause(800);
         while ( CenterConvert(nc) > 180 )
         {

              // ADJUST DONE
              if ( CenterConvert(nc) < 110 )
              {
                  break;
              }
